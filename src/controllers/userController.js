...
const loginUser = async function (req, res) {
  ...
  if (!user) {
    res.send({ msg: "username or the password is not corerct" })
  } else {
    let payload = { emailId, Id: user._id }
    let token = jwt.sign(payload, "lithium-yashraj");

    res.header("x-auth-token", token)
    res.send({ status: "You have Successful LoggedIn", token: token })
  }
} 
